...
type (
	...
	loginRes      struct {
		generated.Casdoor__LoginResponseData_casdoor_apiLogin
...
func login(hook *base.HttpTransportHookRequest, body *plugins.HttpTransportBody) (*base.ClientResponse, error) {
	...
	go func() {
		...
		if !result.Casdoor_apiLogin.Success {
			hook.Logger().Errorf(result.Casdoor_apiLogin.Msg)
			cancel = true
		}
		loginRes.Casdoor__LoginResponseData_casdoor_apiLogin = result.Casdoor_apiLogin
	}()
